...
import { WINDOW_REPLACEMENT_FAQS } from './faqs';
...
export function FAQ() {
  ...
  return (
    <section
      id="window-faq"
      className="relative py-24 md:py-32 lg:py-40 px-4 sm:px-6 lg:px-8 bg-brand-black w-full overflow-hidden border-t border-white/5"
    >
      <div className="max-w-4xl mx-auto">
        ...
        >
          {WINDOW_REPLACEMENT_FAQS.map((item, index) => {
            const isOpen = openIndex === index;
            const panelId = `window-faq-panel-${index}`;
            const buttonId = `window-faq-button-${index}`;

            return (
              <li key={item.q} className="border-t border-white/10">
                <button
                  id={buttonId}
                  type="button"
                ...
                >
                  <span
                    className={`uppercase tracking-[0.1em] text-base md:text-lg font-sans font-light transition-colors duration-300 ${
                      isOpen
                        ? 'text-gold'
                        : 'text-brand-white group-hover:text-gold'
                    }`}
                  >
                    {item.q}
                  </span>
